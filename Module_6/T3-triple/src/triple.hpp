...
public:
  ...
  /*  Triple(const Triple<T1, T2, T3>& triple){
    ...
    Triple<T1, T2, T3>& operator=(const Triple<T1, T2, T3>& triple){
        first_ = triple.First();
        second_ = triple.Second();
        third_ = triple.Third();
        return *this;
...
template <typename T1, typename T2, typename T3>
bool IsHomogenous(const Triple<T1, T2, T3>& triple){
    bool one = typeid(triple.First()).name()==typeid(triple.Second()).name();
    bool two = typeid(triple.Second()).name()== typeid(triple.Third()).name();
 ...
 template <typename T1, typename T2, typename T3>
    bool operator!=(const Triple<T1, T2, T3> &triple, const Triple<T1, T2, T3> &other_triple)
    {  
        if ((triple.First() != other_triple.First()) || (triple.Second() != other_triple.Second()) 
        || (triple.Third() != other_triple.Third())){
            return true;
        }
        return false;          
    }
    ...
    template <typename T1, typename T2, typename T3>
    std::ostream &operator<<(std::ostream &os, const Triple<T1, T2, T3> &triple){
        os << "[" << triple.First() << ", " << triple.Second() << ", " 
        << triple.Third() << "]";
        return os;
    };
